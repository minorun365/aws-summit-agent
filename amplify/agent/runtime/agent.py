...
- search_summit_knowledge や固定情報だけでは足りない最新情報・外部記事・URL確認が必要な場合だけ web_search を使う。
...
def search_summit_knowledge(query: str) -> str:
    ...
    if not results:
        return "Knowledge Baseを検索しましたが、関連する結果は見つかりませんでした。"
    return "\n\n---\n\n".join(results)
...
def web_search(query: str) -> str:
    ...
    Knowledge Base、固定スケジュール、公式情報だけでは足りない場合に使います。
    既存ツールより優先度は低く、通常は search_summit_knowledge を先に使ってください。
